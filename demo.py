...
def correct_feedback(model,video='0',input_csv='0'):
    import time
    last_spoken = {name: 0 for name in ["Left-wrist","Right-wrist","Left-elbow", "Right-elbow","Left-shoulder", "Right-shoulder", "Left-knee", "Right-knee","Left-ankle","Right-ankle","Left-hip", "Right-hip"]}
    speak_interval = 8 # seconds
    ...
    angle_name_list = ["Left-wrist","Right-wrist","Left-elbow", "Right-elbow","Left-shoulder", "Right-shoulder", "Left-knee", "Right-knee","Left-ankle","Right-ankle","Left-hip", "Right-hip"]
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks is not None:
            ...
            for itr in range(12):
                ...
                if status == "more" and time.time() - last_spoken[angle_name_list[itr]] > speak_interval:
                    speak_async(f"lower the {angle_name_list[itr].replace('-', ' ').lower()}")
                    # engine.runAndWait()
                    last_spoken[angle_name_list[itr]] = time.time()
                elif status == "less" and time.time() - last_spoken[angle_name_list[itr]] > speak_interval:
                    speak_async(f"lift the {angle_name_list[itr].replace('-', ' ').lower()}")
                    # engine.runAndWait()
                    last_spoken[angle_name_list[itr]] = time.time()
